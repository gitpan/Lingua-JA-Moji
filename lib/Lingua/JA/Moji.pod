...
and will attempt to convert any romanization it sees into kana. The
romanization is based on the behaviour of the Microsoft IME. To
...
Given an argument, return whether it is a legitimate style of
romanization.
...
Functions for handling the "wide ascii" which is sometimes used in
Japan. The "wide ascii" is a legacy of bitmapped fonts which has
survived into the present day. "Wide ascii" characters were special
bitmapped font characters created to be the same size as one kanji or
kana character.
